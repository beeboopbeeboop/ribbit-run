...
export function QuizOverlay(){
  ...
  return (
    <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,.45)', display:'grid', placeItems:'center', padding:24 }}>
      <div
        style={{
          background:'white', borderRadius:20, padding:24, width:560,
          boxShadow:'0 16px 40px rgba(0,0,0,.28)', border:'2px solid #0f172a10',
          animation: wrongShake ? 'shakeX .5s ease both' : undefined
        }}
        onAnimationEnd={()=> wrongShake && setWrongShake(false)}
      >
        ...
        <div style={{ display:'grid', gap:12 }}>
          {quiz.choices.map((c, i) => (
            <button
              key={c}
              onClick={()=>{
                if(c===quiz.correct){ answer(c) } else { setWrongShake(true); answer(c) }
              }}
